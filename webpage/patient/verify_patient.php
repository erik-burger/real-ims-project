...
if(isset($_GET['email']) && !empty($_GET['email']) AND isset($_GET['verification_hash']) && !empty($_GET['verification_hash'])){	
	...
	$sql_search = "SELECT * FROM patient WHERE email='$email' AND verification_hash='$verification_hash' ";
    ...
    if($search){
    	if(mysqli_num_rows($search)==1){
    		...
    		if($status == 0){
        		...
        		$sql_update = "UPDATE patient SET verified = '1' WHERE email='$email' AND verification_hash='$verification_hash'";
